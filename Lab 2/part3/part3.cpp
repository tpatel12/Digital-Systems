...
//send a bit of data to shift register while raising and lwoering clock pin
...
//Sends a byte of data to shift register and then toggles latch pin
void sendByte(int data) {
  for (int i = 7; i >= 0; i--) { 
    sendBit((data >> i) & 1); // Send the current bit to the shift register
  }

  PORTB |= (1 << LATCH_PIN); // Set the latch pin to high
  PORTB &= ~(1 << LATCH_PIN); // Set the latch pin to low

  
}

//Set a bit of any register
...
//Button press increments the current number displayed
void handle_button(){
  num += 1;
  if(num > 9){
    num = 0;
...
void lights(){
  ...
  int* col7[] = {col7b, col7a};
  int* row7[] = {row7b, row7a};
  pattern_length[7] = 2;
